...
class Xsdk(BundlePackage):
    ...
    depends_on('sundials@5.0.0-dev.2~int64+hypre', when='@develop')
    ...
    depends_on('amrex@19.10+sundials', when='@0.5.0 %intel')
    depends_on('amrex@19.10+sundials', when='@0.5.0 %gcc')
    ...
    depends_on('ginkgo@develop ~openmp', when='@0.5.0 +ginkgo')
    depends_on('ginkgo@develop ~openmp+cuda', when='@0.5.0 +cuda +ginkgo')
